		...
		for (i = 0, s = (ticket_node_t *)(paging.curPos); s != list && i < paging.pageSize; i++, s = s->next)
		{
			...
			if (buf.status == 1)
				printf("\t\t\t  %-4d       %-6d   %-6d %4d %4d %10s\n", s->data.id,
				 s->data.schedule_id, s->data.seat_id, buf.row + 1, buf.column + 1, Status(s->data.status));
		}
...
void Sale_UI_MgtEntry()
{
	...
	do {
		...
		printf("\n\t总数:%d\t\t\t\t\t\t\t\t\t\t%d/%d \n", paging.totalRecords, Pageing_CurPage(paging), Pageing_TotalPages(paging));
		printf("      =========================================================================================\n");
		printf("\t\t  1.上一页    2.下一页    3.查询剧目    4.显示演出计划     5.返回  \n");
